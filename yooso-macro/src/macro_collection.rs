...
pub fn collection(table_metadata: CollectionMeta, mut strucc: ItemStruct) -> TokenStream {
    ...
    // Generate a list of field identifiers. This is used for the insert generator
    ...
    let field_ids = fields
        ...
        .map(|field| {
            ...
            // For Uuid fields, we will call 'to_string()' on the field value
            // to convert it to a string before inserting into the database.
            match &field.ty {
                syn::Type::Path(type_path) if type_path.path.is_ident("Uuid") => quote! { #ident.to_string() },
                _ => quote! { #ident }
            }
        })
        .collect::<Vec<_>>();
